...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for step in range(n):
        if sample is None:
            ...
            # k=1 is default for random.choices()
...
def iterate_pagerank(corpus, damping_factor):
    ...
    while repeat:
        ...
        for page in pagerank:
            ...
            for one_page in corpus:
                # We consider each one possible page that links to current page
                if page in corpus[one_page]:
                    total += pagerank[one_page] / len(corpus[one_page])
                # A page that has no links is interpreted as having one link for every page (including itself)
                if not corpus[one_page]:
                    total += 1 / len(corpus)
